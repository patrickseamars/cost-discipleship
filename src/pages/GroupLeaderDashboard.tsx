...
export default function GroupLeaderDashboard() {
  const { user, profile, loading, hasAccess, isGroupLeader, isAdmin } = useRequireAuth('group_leader');
  ...
  const fetchGroupData = async () => {
    setLoadingData(true);
    console.log('🏢 Fetching group data for user:', profile?.id, 'Role:', profile?.role);
    ...
    try {
      ...
      if (isAdmin) {
        ...
        console.log('👑 Admin accessing groups. Found:', groupData ? 1 : 0, 'groups');
      } else {
      ...
      if (membersData && membersData.length > 0) {
        ...
        const { data: progressData, error: progressError } = await supabase
          .from('user_progress')
          .select('user_id, section_key, day_number, completed_at, is_completed')
  ...
  const assignMemberToGroup = async (userId: string, userName: string) => {
    ...
    console.log('📝 Assigning user to group:', { userId, userName, groupId: groupData.id });
  ...
  const getMemberStats = (memberId: string) => {
    const memberProgressData = memberProgress.filter(p => p.user_id === memberId);
    const completedCount = memberProgressData.filter(p => p.is_completed).length;
    ...
    return {
      ...
      completionRate,
      lastActivity: memberProgressData.length > 0 ? 
        new Date(Math.max(...memberProgressData.map(p => new Date(p.completed_at).getTime()))) : 
        null
    };
  };
  ...
  const groupStats = {
    ...
    activeMembers: members.filter(member => {
      ...
      return stats.lastActivity && 
        (Date.now() - stats.lastActivity.getTime()) < (7 * 24 * 60 * 60 * 1000); // Active in last 7 days
    }).length,
    avgCompletion: members.length > 0 ? 
      Math.round(members.reduce((sum, member) => sum + getMemberStats(member.id).completionRate, 0) / members.length) : 0
  };
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <Card className="w-full max-w-md">
          <CardHeader className="text-center">
            ...
            <CardTitle>{isAdmin ? 'No Groups Found' : 'No Group Assigned'}</CardTitle>
            <CardDescription>
              {isAdmin ? 
  ...
  const OverviewTab = () => (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">Monitor your group's progress and engagement</p>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="text-center">
              <div className="text-3xl font-bold text-green-600">{groupStats.activeMembers}</div>
              <div className="text-sm text-gray-600">Active This Week</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-purple-600">{groupStats.avgCompletion}%</div>
                    ...
                    <div key={index} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-600">
                          Completed {progress.section_key} - Day {progress.day_number}
                        </p>
                      </div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Manage and communicate with your group members</p>
        </div>
        ...
        <CardContent className="p-0">
          {members.length === 0 ? (
            <div className="p-8 text-center">
              <Users className="w-12 h-12 text-gray-400 mx-auto mb-4" />
              <p className="text-gray-500 text-lg font-medium">No members in this group</p>
              <p className="text-gray-400 text-sm">Members will appear here when assigned by an admin</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          <Button 
                            ...
                            onClick={() => removeMemberFromGroup(member.id, `${member.first_name} ${member.last_name}`)}
                          >
  ...
  const ProgressTab = () => (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">Monitor individual and group progress through COST training</p>
      </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      <div className="font-medium">{stats.completedExercises}</div>
                      <div className="text-gray-600">Completed</div>
                    </div>
                    <div>
                      <div className="font-medium">
                        {stats.lastActivity ? 
                          `${Math.floor((Date.now() - stats.lastActivity.getTime()) / (24 * 60 * 60 * 1000))}d ago` : 
                          'Never'
                        }
                      </div>
  ...
  const EngagementTab = () => {
    const needsAttention = members.filter(member => {
      ...
      const daysSinceActivity = stats.lastActivity ? 
        Math.floor((Date.now() - stats.lastActivity.getTime()) / (24 * 60 * 60 * 1000)) : 999;
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-gray-600">Identify members who need encouragement and support</p>
        </div>
          ...
          <CardContent>
            {needsAttention.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-green-700 font-medium">Great job! All members are engaged.</p>
                <p className="text-gray-600 text-sm">No members need immediate attention.</p>
              </div>
            ) : (
              <div className="space-y-4">
                  ...
                  return (
                    <div key={member.id} className="flex items-center justify-between p-4 border border-orange-200 bg-orange-50 rounded-lg">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          {daysSinceActivity > 30 ? 'Inactive for over 30 days' : 
                           daysSinceActivity > 7 ? `Inactive for ${daysSinceActivity} days` : 
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-3xl font-bold text-green-600">
                {members.filter(member => getMemberStats(member.id).completionRate > 60).length}
              </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-3xl font-bold text-red-600">
                {members.filter(member => getMemberStats(member.id).completionRate < 20).length}
              </div>
              <p className="text-gray-600">Members with less than 20% completion</p>
            </CardContent>
          </Card>
        </div>
      </div>
                ...
                <div className="space-y-2">
                  {availableUsers.map((user) => (
                    <div key={user.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      ...
                      <Button
                        size="sm"
                        onClick={() => assignMemberToGroup(user.id, `${user.first_name} ${user.last_name}`)}
                      >
                        Add to Group
                      </Button>
                    </div>
        ...
        <div className="px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="font-semibold text-gray-900">Group Leader Dashboard</h1>
                <p className="text-xs text-gray-500">Manage Your Group</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <Badge variant="outline" className={isAdmin ? "bg-red-50 text-red-700" : "bg-green-50 text-green-700"}>
