...
export default function AdminDashboard() {
  ...
  const deleteUser = async (userId: string, userName: string) => {
    ...
    try {
      ...
      if (ledGroups && ledGroups.length > 0) {
        console.log(`User leads ${ledGroups.length} groups, unassigning leadership...`);
        ...
        if (unleaderError) {
          console.error('❌ Error unassigning group leadership:', unleaderError);
          alert(`Failed to unassign group leadership: ${unleaderError.message}`);
          return;
        }
      ...
      console.log('⚠️ Note: Auth user still exists in Supabase Auth (requires manual cleanup)');
      alert(
  ...
  const deleteGroup = async (groupId: string, groupName: string) => {
    if (!confirm(`Are you sure you want to delete group "${groupName}"? All members will be unassigned from this group. This action cannot be undone.`)) {
      return;
    }
  ...
  const updateUser = async (userId: string, updates: { first_name: string; last_name: string; email: string; role: string }) => {
  ...
  const updateGroup = async (groupId: string, updates: { name: string; description: string; leader_id?: string }) => {
