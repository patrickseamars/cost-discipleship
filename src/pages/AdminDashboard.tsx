...
export default function AdminDashboard() {
	const { user, profile, loading, hasAccess, isAdmin } =
	...
	const deleteUser = async (userId: string, userName: string) => {
		...
		try {
			// First check if user is a group leader
			const { data: ledGroups, error: checkError } = await supabase
				.from("groups")
				.select("id, name")
				.eq("leader_id", userId);

			if (checkError) {
				console.error("❌ Error checking group leadership:", checkError);
				alert(`Failed to check group leadership: ${checkError.message}`);
				return;
			}

			// If user leads any groups, unassign them as leader first
			if (ledGroups && ledGroups.length > 0) {
				console.log(
					`User leads ${ledGroups.length} groups, unassigning leadership...`
				);

				const { error: unleaderError } = await supabase
					.from("groups")
				...
				if (unleaderError) {
					console.error(
						"❌ Error unassigning group leadership:",
						unleaderError
					);
					alert(
						`Failed to unassign group leadership: ${unleaderError.message}`
					);
					return;
				}

				console.log("✅ Successfully unassigned group leadership");
			}

			// Now delete the user - both profile and auth user
			...
			// Note: Deleting from auth.users requires service-level permissions
			// For security, we don't expose the service key to the frontend
			// The user's profile is deleted, which prevents them from using the app
			// But they may still appear in Supabase Auth until manually removed

			console.log(
				"⚠️ Note: Auth user still exists in Supabase Auth (requires manual cleanup)"
			);
			alert(
				"User profile deleted successfully!\n\n" +
					"Note: The user will not be able to use the app anymore, but their " +
					"authentication record still exists in Supabase Auth and may need " +
					"to be manually removed from the Supabase dashboard if desired."
			);

			// Refresh both users and groups lists
			fetchUsers();
			fetchGroups();
		} catch (error) {
			console.error("💥 Exception while deleting user:", error);
			alert(`Exception while deleting user: ${error.message}`);
		}
	};
	...
	const deleteGroup = async (groupId: string, groupName: string) => {
		if (
			!confirm(
				`Are you sure you want to delete group "${groupName}"? All members will be unassigned from this group. This action cannot be undone.`
			)
		) {
			return;
		}
		...
		try {
			...
			const { data: groupMembers, error: checkError } = await supabase
				.from("profiles")
				.select("id, first_name, last_name")
				.eq("group_id", groupId);

			if (checkError) {
				console.error("❌ Error checking group members:", checkError);
				alert(`Failed to check group members: ${checkError.message}`);
				return;
			}

			console.log("👥 Found group members:", groupMembers);

			// If there are members, unassign them first
			if (groupMembers && groupMembers.length > 0) {
				console.log(`Unassigning ${groupMembers.length} members from group...`);
				...
				if (unassignError) {
					...
					alert(`Failed to unassign members: ${unassignError.message}`);
					return;
				}

				console.log("✅ Successfully unassigned all members");
			}
			...
			if (deleteError) {
				...
				// Refresh both groups and users lists
				fetchGroups();
				fetchUsers();
			...
			alert(`Exception while deleting group: ${error.message}`);
		}
	};
	...
	const updateUser = async (
		...
		try {
			...
			alert(`Exception while updating user: ${error.message}`);
		}
	};

	const updateGroup = async (
		groupId: string,
		updates: { name: string; description: string; leader_id?: string }
	) => {
		console.log("✏️ Updating group:", { groupId, updates });
		...
		try {
			...
			alert(`Exception while updating group: ${error.message}`);
		}
	};
	...
	const fetchAnalytics = async () => {
		...
		try {
			...
			const groupLeaders =
				usersData?.filter((user) => user.role === "group_leader").length || 0;
			...
			const recentUsers =
				usersData?.filter((user) => new Date(user.created_at) > sevenDaysAgo) ||
				[];
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-gray-600">
						Monitor your COST Discipleship platform
					</p>
				</div>
				<Button
	...
	const CreateGroupForm = () => {
		...
		const handleSubmit = async (e: React.FormEvent) => {
			...
			await createGroup(
				groupName,
				groupDescription,
				selectedLeader === "none" ? undefined : selectedLeader || undefined
			);
			setSubmitting(false);
		...
		const groupLeaders = users.filter(
			(user) => user.role === "group_leader" || user.role === "admin"
		);
				...
				<CardContent className="p-0">
					{loadingGroups ? (
						...
						<div className="p-8 text-center">
							...
							<p className="text-gray-400 text-sm">
								Create your first Discipleship group to get started
							</p>
						</div>
					) : (
						<Table>
											...
											<div className="flex items-center gap-2">
												...
												<Button
													...
													onClick={() => {
														...
														// Ensure users are loaded for leader selection
	...
	const ManageMembersModal = () => {
		...
		const groupMembers = users.filter(
			(user) => user.group_id === selectedGroup.id
		);
					...
					<CardHeader>
						<div className="flex items-center justify-between">
							<div>
								...
								<CardDescription>
									{selectedGroup.name} - {groupMembers.length} member
									...
									{availableUsers.map((user) => (
										...
										>
											...
											<Button
												size="sm"
												onClick={() =>
													assignUserToGroup(user.id, selectedGroup.id)
												}
											>
												Add to Group
											</Button>
	...
	const EditUserModal = () => {
		...
		const [role, setRole] = useState(selectedUser.role || "member");
		const [submitting, setSubmitting] = useState(false);
	...
	const EditGroupModal = () => {
		...
		const [leaderId, setLeaderId] = useState(selectedGroup.leader_id || "none");
		const [submitting, setSubmitting] = useState(false);
					...
					<CardHeader>
						<div className="flex items-center justify-between">
							<div>
								...
								<CardDescription>
									Update group information and leader
								</CardDescription>
							</div>
							<Button
								variant="outline"
		...
		<div className="space-y-6">
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Tools to debug RLS policies and database access
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
					...
					<div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
						<p className="text-sm text-yellow-800">
							<strong>Note:</strong> This debug section helps identify database
							permission issues. Check the browser console for detailed results
