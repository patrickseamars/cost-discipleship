...
const SectionsLayout = () => {
		...
		if (currentView && currentView.includes("-")) {
			...
			if (parts.length >= 2) {
				...
				if (!isNaN(dayNum)) {
					...
					const isCompleted = completionStorage.isDayComplete(
						sectionKey,
						dayNum
					);
	...
	const getTotalProgress = () => {
		...
		const totalCompleted = Object.values(allCompletions).reduce(
			...
			for (let day = 1; day <= section.totalDays; day++) {
				if (!completionStorage.isDayComplete(section.key, day)) {
					return {
						...
						data: dailyExercisesData.sections[
							section.key
						]?.daily_exercises?.find((ex) => ex.day === day),
					};
				}
			}
		}
	...
	const saveReflections = (dayId: string) => {
		...
		if (
			reflectionAnswers[dayId] &&
			Object.keys(reflectionAnswers[dayId]).length > 0
		) {
	...
	const saveQuestions = (dayId: string) => {
		...
		if (
			questionAnswers[dayId] &&
			Object.keys(questionAnswers[dayId]).length > 0
		) {
		...
		<div className="w-64 bg-white border-r border-gray-200 h-full flex flex-col">
			...
			<div className="p-4 border-b border-gray-100">
				<div className="flex items-center justify-between">
					<h2 className="font-semibold text-gray-900">COST Discipleship</h2>
					<Button
						variant="ghost"
						size="sm"
						className="md:hidden"
								...
								<div key={section.key}>
									...
									{isExpanded && (
										<div className="ml-6 space-y-1">
											...
											<Button
												variant={
													currentView === `${section.key}-overview`
														? "secondary"
														: "ghost"
												}
												size="sm"
											...
											<Button
												variant={
													currentView === `${section.key}-week-review`
														? "secondary"
														: "ghost"
												}
												size="sm"
											...
											<Button
												variant={
													currentView === `${section.key}-summary`
														? "secondary"
														: "ghost"
												}
												size="sm"
							...
							<CardContent>
								<p className="text-gray-600">
									{currentDay.data.content ||
										"Continue your spiritual development with today's exercises."}
								</p>
					...
					<CardContent>
						<div className="grid gap-4">
							{sections.slice(0, 4).map((section) => {
								...
								const completedDays = completionStorage.getCompletedDays(
									section.key
								);
								return (
									<div
										...
										<div className="flex-1">
											<div className="flex items-center justify-between mb-1">
												...
												<span className="text-sm text-gray-500">
													{completedDays.length}/{section.totalDays}
												</span>
											</div>
				...
				<Card>
					...
					<CardContent>
						<div className="space-y-3">
							{Object.entries(assessmentStorage.getAllAssessments())
								.slice(0, 2)
								.map(([sectionKey, assessments]) => (
									<div key={sectionKey}>
										{assessments.initial && (
											<div className="flex items-center justify-between py-2">
												<div>
													<div className="font-medium capitalize">
														Initial {sectionKey} Assessment
													</div>
													<div className="text-sm text-gray-500">
														{new Date(
															assessments.initial.completedAt
														).toLocaleDateString()}
													</div>
												</div>
												<Badge variant="secondary">
													{Math.round(
														assessments.initial.results.percentageScore
													)}
													%
												</Badge>
											</div>
										...
										{assessments.final && (
											<div className="flex items-center justify-between py-2">
												<div>
													<div className="font-medium capitalize">
														Final {sectionKey} Assessment
													</div>
													<div className="text-sm text-gray-500">
														{new Date(
															assessments.final.completedAt
														).toLocaleDateString()}
													</div>
												</div>
												<Badge variant="default">
													{Math.round(
														assessments.final.results.percentageScore
													)}
													%
												</Badge>
											</div>
							...
							{Object.keys(assessmentStorage.getAllAssessments()).length ===
								0 && (
	...
	const AssessmentsView = () => {
		...
		if (selectedSection) {
			...
			const evaluationItems =
				sectionData?.daily_exercises?.find(
					(ex) => ex.day === 1 && ex.type === "assessment"
				)?.evaluation_items || [];
							...
							<Card>
								...
								<CardContent className="space-y-6">
									...
									<div className="text-center space-y-2">
										...
										<div className="text-sm text-gray-500">
											Average Score ({assessments.initial.results.totalScore} /{" "}
											{assessments.initial.results.maxPossibleScore} total
											points)
										</div>
									</div>
									...
									{/* Strongest Areas */}
									{assessments.initial.results.strongestAreas &&
										assessments.initial.results.strongestAreas.length > 0 && (
											<div>
									...
									{/* Growth Areas */}
									{assessments.initial.results.weakestAreas &&
										assessments.initial.results.weakestAreas.length > 0 && (
											<div>
												<h4 className="font-semibold text-sm mb-3 flex items-center gap-2">
							...
							<Card>
								...
								<CardContent className="space-y-6">
									...
									<div className="text-center space-y-2">
										...
										<div className="text-sm text-gray-500">
											Average Score ({assessments.final.results.totalScore} /{" "}
											{assessments.final.results.maxPossibleScore} total points)
										</div>
										{assessments.initial && (
											<div className="flex items-center justify-center gap-2 mt-2">
												{assessments.final.results.averageScore >
												assessments.initial.results.averageScore ? (
													<>
														<TrendingUp className="w-4 h-4 text-green-600" />
									...
									{assessments.final.results.strongestAreas &&
										assessments.final.results.strongestAreas.length > 0 && (
											<div>
												<h4 className="font-semibold text-sm mb-3 flex items-center gap-2">
													<TrendingUp className="w-4 h-4 text-green-600" />
									...
									{/* Final Growth Areas */}
									{assessments.final.results.weakestAreas &&
										assessments.final.results.weakestAreas.length > 0 && (
											<div>
												<h4 className="font-semibold text-sm mb-3 flex items-center gap-2">
													<AlertCircle className="w-4 h-4 text-orange-600" />
		...
		return (
			<div className="max-w-4xl mx-auto space-y-8">
				<div className="space-y-2">
					...
					<p className="text-gray-600">
						Review your progress and growth over time
					</p>
				</div>
								...
								<div className="space-y-4">
									{assessments.initial && (
										<div>
											...
											<Progress
												value={assessments.initial.results.percentageScore}
												className="h-2 mb-2"
											/>
									...
									{assessments.final && (
										<div>
											...
											<Progress
												value={assessments.final.results.percentageScore}
												className="h-2 mb-2"
											/>
											<div className="text-xs text-gray-500">
												Completed{" "}
						...
						<Card>
							<CardContent className="text-center py-8">
								...
								<p className="text-gray-500 mb-4">
									Complete your first section to see assessment results here.
								</p>
	...
	const DayView = (dayId: string, isFinalAssessment: boolean = false) => {
		...
		const dayData = exerciseData?.daily_exercises?.find(
			(ex) => ex.day === dayNum
		);
		...
		const isCompleted = isFinalAssessment
			? false
			: completionStorage.isDayComplete(sectionKey, dayNum);
		const isEditingMode = isFinalAssessment
			? true
			: isEditing[dayId] ?? !isCompleted;
			...
			<div className="max-w-4xl mx-auto space-y-8">
				...
				<div className="space-y-2">
					<div className="flex items-center gap-2 text-sm text-gray-500">
						...
						<span>•</span>
						<span>
							{isFinalAssessment ? "Final Assessment" : `Day ${dayNum}`}
						</span>
						{isCompleted && (
							<>
								<span>•</span>
				...
				<div className="prose prose-lg max-w-none">
										...
										<div key={index} className="space-y-3">
											...
											<div className="flex gap-1 flex-wrap">
												{Array.from({ length: 10 }, (_, i) => {
													...
													const isDisabled = isFinalAssessment
														? false
														: !isEditingMode;
													...
													return (
														<Button
															...
															onClick={() =>
																handleAssessmentRating(dayId, index, rating)
															}
															disabled={isDisabled}
														>
															{rating}
											...
											{currentRatings[index] && (
												<p className="text-xs text-gray-500">
													You rated this: {currentRatings[index]}/10
												</p>
											)}
										</div>
									))}
									...
									<div className="mt-6 p-4 bg-blue-50 rounded-lg">
										<div className="flex justify-between items-center mb-2">
											...
											<span className="text-sm text-blue-700">
												{Object.keys(currentRatings).length} of{" "}
												{dayData.evaluation_items.length} completed
											</span>
										</div>
										<Progress
											value={
												(Object.keys(currentRatings).length /
													dayData.evaluation_items.length) *
												100
											}
											className="h-2 mb-2"
										/>
												...
												<CardContent className="space-y-6">
													{(() => {
														...
														const getOverallRating = (percentage) => {
															...
															return {
																label: "Needs Significant Growth",
																color: "text-red-600",
															};
														};
																...
																<div className="text-center space-y-2">
																	...
																	<div
																		className={`text-lg font-semibold ${
																			getOverallRating(results.percentageScore)
																				.color
																		}`}
																	>
																		{
																...
																{results.completedItems <
																	...
																	<div className="text-center p-4 bg-blue-50 rounded-lg">
																		<p className="text-sm text-blue-700">
																			Complete all ratings to see your full
																			assessment results and reflection prompts.
																		</p>
																	</div>
																)}
															</>
						...
						<Card className="mb-8">
							<CardHeader>
								<div className="flex items-center justify-between">
									...
									{isEditingMode &&
										Object.keys(currentQuestions).length > 0 && (
											<Button
												onClick={() => {
													saveQuestions(dayId);
													setIsEditing((prev) => ({ ...prev, [dayId]: false }));
									...
									{!isEditingMode && (
										<Button
											onClick={() =>
												setIsEditing((prev) => ({ ...prev, [dayId]: true }))
											}
											variant="outline"
											size="sm"
											className="gap-2"
										...
										<div key={index} className="space-y-3">
											...
											{isEditingMode ? (
												<Textarea
													...
													onChange={(e) =>
														handleQuestionChange(dayId, index, e.target.value)
													}
													rows={3}
												/>
											) : (
					...
					{dayData.type === "assessment" &&
						dayData.reflection_prompts &&
						dayData.reflection_prompts.length > 0 &&
						Object.keys(currentRatings).length ===
							dayData.evaluation_items.length && (
							<Card className="mb-8">
								<CardHeader>
									...
									<CardDescription>
										Complete your reflections to finish the assessment
									</CardDescription>
								</CardHeader>
								<CardContent>
									<div className="space-y-6">
										{dayData.reflection_prompts.map((prompt, index) => (
											<div key={index} className="space-y-3">
												...
												<Textarea
													...
													onChange={(e) =>
														handleReflectionChange(dayId, index, e.target.value)
													}
													rows={3}
										...
										{(() => {
											const completedReflections = Object.keys(
												currentReflections
											).filter(
												(key) =>
													currentReflections[parseInt(key)]?.trim().length > 0
											).length;
											const isReflectionsComplete =
												completedReflections ===
												dayData.reflection_prompts.length;
											const isRatingsComplete =
												Object.keys(currentRatings).length ===
												dayData.evaluation_items.length;
											const isReadyToSubmit =
												isRatingsComplete && isReflectionsComplete;
													...
													{!isReadyToSubmit && (
														<div className="text-center p-4 bg-orange-50 rounded-lg">
															<p className="text-sm text-orange-700">
																Complete all reflection questions to submit your
																assessment.
															</p>
															<p className="text-xs text-gray-600 mt-1">
																Reflections completed: {completedReflections} of{" "}
																{dayData.reflection_prompts.length}
															</p>
														</div>
													)}
													...
													{isReadyToSubmit && !isCompleted && (
														<div className="text-center p-4 space-y-3">
															<p className="text-sm text-green-700">
																✓ All questions completed! Ready to submit your
																assessment.
															</p>
															<Button
																onClick={() => {
													...
													{isCompleted && (
														<div className="text-center p-4 bg-green-50 rounded-lg">
															<p className="text-sm text-green-700 font-medium">
																✓ Assessment Submitted Successfully! Your
							...
							<CardContent>
								<div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
									<p className="text-sm text-blue-900">
										Based on your assessment, focus on your growth opportunities
										while maintaining your strengths. Consider setting specific
										goals for areas scoring below 7/10.
									</p>
					...
					<Card>
						<CardContent className="p-8">
							<p className="text-gray-500">
								Overview data not found for this section.
							</p>
						</CardContent>
					</Card>
				</div>
				...
				{sectionData.soap_method && (
					<Card>
						...
						<CardContent className="space-y-4">
							<p className="text-gray-700 leading-relaxed">
								{sectionData.soap_method.description}
							</p>
							<div className="grid md:grid-cols-2 gap-4">
								<div className="p-4 bg-blue-50 rounded-lg">
									<h4 className="font-semibold text-blue-900 mb-2">
										S - {sectionData.soap_method.S}
									</h4>
								</div>
								<div className="p-4 bg-green-50 rounded-lg">
									<h4 className="font-semibold text-green-900 mb-2">
										O - {sectionData.soap_method.O}
									</h4>
								</div>
								<div className="p-4 bg-yellow-50 rounded-lg">
									<h4 className="font-semibold text-yellow-900 mb-2">
										A - {sectionData.soap_method.A}
									</h4>
								</div>
								<div className="p-4 bg-purple-50 rounded-lg">
									<h4 className="font-semibold text-purple-900 mb-2">
										P - {sectionData.soap_method.P}
									</h4>
								</div>
							</div>
						</CardContent>
						...
						<CardContent className="space-y-6">
							...
							{sectionData.meeting.discussion_points && (
								<div>
									...
									<div className="space-y-3">
										{sectionData.meeting.discussion_points.map(
											(question, index) => (
												<div
													key={index}
													className="p-3 bg-yellow-50 rounded-lg border-l-4 border-yellow-300"
												>
			...
			return (
				<div className="max-w-4xl mx-auto">
					<Card>
						<CardContent className="p-8">
							<p className="text-gray-500">
								Summary data not found for this section.
							</p>
						</CardContent>
					</Card>
				</div>
	...
	const WeekReviewView = (viewId: string) => {
		...
		const initialAssessmentData = sectionData.daily_exercises?.find(
			(ex) => ex.day === 1 && ex.type === "assessment"
		);
				...
				<div className="max-w-4xl mx-auto">
					<Card>
						<CardContent className="p-8 text-center">
							...
							<p className="text-gray-500">
								This section doesn't have an assessment to review.
							</p>
						</CardContent>
					</Card>
				</div>
		...
		return (
			<div className="max-w-4xl mx-auto space-y-8">
				<div className="space-y-2">
					<h1 className="text-3xl font-bold text-gray-900">
						Week Review - {section.name}
					</h1>
					<p className="text-gray-600">
						Compare your initial and final assessments to see your growth
					</p>
				</div>
							...
							<div className="space-y-2">
								{hasInitialAssessment ? (
									<>
										...
										<Badge variant="secondary">
											{Math.round(
												assessmentComparison.initial.results.percentageScore
											)}
											%
										</Badge>
									</>
							...
							<div className="space-y-2">
								{hasFinalAssessment ? (
									<>
										...
										<Badge variant="default">
											{Math.round(
												assessmentComparison.final.results.percentageScore
											)}
											%
										</Badge>
									</>
					...
					<Card>
						...
						<CardContent className="space-y-6">
							...
							<div className="text-center space-y-2">
								<div className="grid grid-cols-2 gap-4">
									<div className="space-y-1">
										<div className="text-2xl font-bold text-gray-600">
											{assessmentComparison.initial.results.averageScore.toFixed(
												1
											)}
										</div>
									...
									<div className="space-y-1">
										<div className="text-2xl font-bold text-blue-600">
											{assessmentComparison.final.results.averageScore.toFixed(
												1
											)}
										</div>
								...
								<div className="flex items-center justify-center gap-2 mt-2">
									{assessmentComparison.final.results.averageScore >
									assessmentComparison.initial.results.averageScore ? (
										<>
											<TrendingUp className="w-4 h-4 text-green-600" />
											<span className="text-sm font-medium text-green-600">
												+
									...
									<div key={index} className="space-y-2">
										<div className="flex items-start justify-between">
											...
											<div className="flex items-center gap-2">
												...
												<div
													className={`flex items-center gap-1 px-2 py-1 rounded text-xs ${getChangeColor(
														item.changeType
													)}`}
												>
										...
										<div className="flex gap-4">
											<div className="flex-1">
												...
												<Progress
													value={(item.initialRating / 10) * 100}
													className="h-3"
												/>
												<div className="text-xs text-gray-500 mt-1 font-mono">
													{item.initialRating}/10
												</div>
											</div>
											<div className="flex-1">
												<div className="text-xs font-medium text-gray-500 mb-2">
								...
								<div className="flex items-center justify-between">
									<div>
										<p className="text-sm font-medium">
											Want to retake the final assessment?
										</p>
										<p className="text-xs text-gray-500">
						...
						<CardContent>
							<div className="text-center p-4 bg-yellow-50 rounded-lg">
								<p className="text-sm text-yellow-700 mb-4">
									You need to complete the initial assessment (Day 1) before you
									can do the week review comparison.
								</p>
						...
						<CardContent>
							<div className="space-y-4">
								<p className="text-sm text-gray-600">
									You completed your initial assessment on Day 1. Now it's time
									to reassess yourself after a week of growth and see how you've
									progressed!
								</p>
								<Button
									onClick={() => setCurrentView(`${sectionKey}-final`)}
					...
					<div className="flex items-center justify-between h-16">
						<div className="flex items-center gap-4">
							...
							<div>
								...
								<p className="text-xs text-gray-500">
									Cultivating Obedience through Scripture & Training
								</p>
							</div>
						</div>
			...
			<div className="flex h-[calc(100vh-4rem)]">
				...
				<div className="flex-1 overflow-auto">
					<div className="p-6 sm:p-8">
						...
						{currentView.includes("-week-review") &&
							WeekReviewView(currentView)}
						{currentView.includes("-final") && DayView(currentView, true)}{" "}
						{/* Handle final assessment as fresh assessment */}
						{currentView.includes("-") &&
							!currentView.includes("-overview") &&
							!currentView.includes("-summary") &&
							!currentView.includes("-week-review") &&
							!currentView.includes("-final") &&
